    ...
    with tab2:
        ...
        if file:
            ...
            for _, row in df.iterrows():
                ...
                c.execute(
                    "SELECT 1 FROM users WHERE role='OPERATOR' AND npsn=?",
                    (npsn,)
                )
                if c.fetchone():
                    duplicate_npsn += 1
                ...
                try:
                    c.execute(
                        """
                        INSERT INTO users
                        (username, password, role, npsn)
                        VALUES (?,?,?,?)
                        """,
                        (
                            username,
                            hash_pw(DEFAULT_PASSWORD),
                            "OPERATOR",
                            npsn
                        )
                    )
